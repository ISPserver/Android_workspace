...
//제이쿼리의 JQuery ajax처럼, 안드로이드에서도 웹서버와 통신시
//자주 사용되는 비동기 객체가 지원됨. but 2019말에 Deprecated
//AsyncTask <비동기로 작업할때 사용될 파라미터(url), 진행상황 데이터, 응답데이터(웹응답데이터)>
public class MyAsync extends AsyncTask<String, Void, String> {
   ...
   //비동기 요청(쓰레드로 수행)
    ...
    //doInBackground() 메서드에서의 반환값이 아래의 onPostExecute()메서드 인수로 전달(시스템에 의해)
    protected void onPostExecute(String data) {
        Log.d(TAG, "background요청 후 전달받은 데이터는"+data);

        //제이슨 배열을 어댑터의 리스트에 넣기
        try {
            ...
            for(int i=0; i<jsonArray.length(); i++){
                ...
                //웹서버에서 이미지 가져와서 비트맵으로 전환
                ImageDownLoaderAsync downAsync = new ImageDownLoaderAsync(gallery,galleryFragment);
                downAsync.execute("http://192.168.35.71:7777/images/"+gallery.getFilename());
